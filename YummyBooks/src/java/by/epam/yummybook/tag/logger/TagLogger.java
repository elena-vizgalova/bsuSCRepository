package by.epam.yummybook.tag.logger;
...
/**
 * Logger class for customer tags classes.
 * @author Elena Vizgalova
 */
public class TagLogger {
    private static TagLogger instance;
    private static Logger logger;

   /**
     * Initializing <code>TagLogger</code>.
     * Sets it's filename, level, <code>PatternLayout</code> with 
     * {@link LoggerPropertyManager#CONVERSION_PATTERN}
     * @param msgSender 
     */
    private TagLogger( Class msgSender ) {
        ...
        try {
            String fileName = loggerPropertyManager.getProperty( 
                    LoggerPropertyManager.CUSTOMERTAG_LOG_FILENAME );
            ...
            String loggerLevelStr = loggerPropertyManager.getProperty( 
                    LoggerPropertyManager.CUSTOMERTAG_LOG_LEVEL );
    ...
    public static synchronized TagLogger getInstance(Class msgSender) {
        if ( instance == null ) {
            instance = new TagLogger( msgSender );
        }
        return instance;
    }
